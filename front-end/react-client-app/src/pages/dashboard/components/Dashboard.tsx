...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (profile.projects.length === 0) {
      ...
      const statList = profile.projects.map(e => {
        if(e.name && e.contentType) {
          return {title: e.name, value: e.contentType} 
        }else return {
          title: '', value:''
        }
  ...
  return (
    <div className="p-4 overflow-y-auto">
      <h2 className="mb-2 font-bold text-primary">Welcome {profile.userAccount.userName}.</h2>
      {/* Overview Cards */}
      <InformationGrid stats={stats} />
      <button onClick={() => setModalOpen(true)} className="btn btn-primary mb-3">Create Project </button>
