...
const putContent: RequestHandler | null = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (validationErrors.length > 0) {
      ...
      const { type, title, body, properties, projectItemId, id } = reqBody;
      if (
        typeof type === "string" &&
        typeof title === "string" &&
        typeof body === "string" &&
        ...
        const savedContent =  await saveContent({
          id, type, title, body, properties, projectItemId 
        });
